...
class Character(Entity):
    ...
    def raycast(self):
        if check_connections(int(self.radius), self, self.player) and self.has_los() and not self.has_moved and not self.has_noticed:
            self.player.block()
    ...
    def has_los(self):
        if vector(self.rect.center).distance_to(self.player.rect.center) < self.radius:
            collisions = [rect.clipline(self.rect.center, self.player.rect.center) for rect in self.collision_rects]
